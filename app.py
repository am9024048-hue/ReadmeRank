...
# 1. إعدادات الصفحة
st.set_page_config(
    page_title="ReadmeRank | README Auditor",
    page_icon="📊",
    layout="wide"
)

# إضافة تنسيق CSS لتنظيم الاتجاهات للغة العربية
st.markdown("""
    <style>
    body, .stApp {
        direction: rtl;
    ...
    .stTextInput, .stRadio, .stFileUploader {
        direction: rtl;
        text-align: right;
    }
    </style>
...
# العنوان الرئيسي والوصف الاحترافي
st.title("ReadmeRank | README Auditor")
st.write("أداة احترافية لتحليل وتقييم جودة ملفات التوثيق (README) للمشاريع البرمجية وضمان تكاملها.")

# 2. طريقة الإدخال بنصوص سليمة وواضحة
st.markdown("---")
input_method = st.radio(
    "اختر مصدر ملف README للتحليل:", 
    ["رابط مستودع GitHub", "رفع ملف README من الجهاز"]
)
...
if input_method == "رابط مستودع GitHub":
    ...
    uploaded_file = st.file_uploader("اختر ملف README من جهازك:", type=["md", "txt"])
...
if analysis_triggered and readme_content:
    st.markdown("---")
    st.subheader("تقرير تحليل وتقييم التوثيق")
    ...
    sections_status = [
        ...
        ("صور العرض (Screenshots / Demo)", has_screenshots, "يقدم نظرة بصرية أو عروض توضيحية للمشروع.")
    ]
