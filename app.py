...
if st.button("🚀 ابدأ الفحص والتحليل الشامل"):
  if not api_key:
    ...
    with st.spinner("🤖 جاري الفحص وتحليل البيانات بسرعة البرق..."):
      try:
        # البحث الذكي عن أول موديل متاح
        models_response = client.models.list()
        available_model = None
        for m in models_response.data:
          if "llama" in m.id or "mixtral" in m.id or "gemma" in m.id:
            available_model = m.id
            break

        if not available_model:
          available_model = "llama-3.3-70b-versatile"

        prompt = (
            "قم بتحليل وتقییم ملف الـ README التالي الخاص بمشروع برمجى."
            " أعطني تقييماً مختصراً لأهم نقاط القوة والضعف وكيفية تحسينه:\n\n"
            + readme_content
        )
        ...
        completion = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=512,  # تم تعديلها لتتطابق مع الحد الأقصى للموديل
        )
