...
if st.button("🚀 ابدأ الفحص الهيكلي الشامل"):
  if not readme_content.strip():
    ...
    with st.spinner("⚡ جاري فحص هيكل الملف وتقييم الأقسام..."):
      # نظام التحليل الهيكلي المحلي السريع
      ...
      # حساب التقييم من 100
      score = 40  ندهدي نقاط أساسية لوجود ملف
      if has_installation:
        score += 20
      if has_usage:
        score += 20
      ...
      st.markdown(
          ...
          ' else '❌ غير متوفر (مهم جداً للمطورين)'}"
      )
      st.markdown(
          f"- **قسم الاستخدام والأمثلة (Usage):** {'✅ متوفر' if has_usage"
          ' else '❌ غير متوفر (يُفضل إضافته)'}"
      )
      st.markdown(
          f"- **شروط الترخيص (License):** {'✅ متوفر' if has_license else"
          ' ❌ غير متوفر (يحتاجه البزنس والشركات)'}"
      )
      st.markdown(
      ...
      st.markdown(
          ...
          ' ⚠️ لا توجد شارات (لإعطاء مظهر احترافي)'}"
      )

      st.write("### 💡 توصيات سريعة لتحسين المشروع:")
      if score < 80:
        st.info(
            "💡 لإبقاء مشروعك جذاباً، احرص على إضافة الأقسام المفقودة وتوضيح"
            " أمثلة برمجية سريعة في بدايته."
        )
      else:
        st.success(
            "🌟 ملف الـ README الخاص بك منظم واحترافي، ويمتلك أساساً قوياً لجذب"
            " المهتمين!"
        )

      # تنبيه للنسخة المدفوعة المستقبلية
      st.write("---")
      st.info(
          "🔒 **هل تريد تحليلاً أعمق واقتراحات تسويقية بالذكاء الاصطناعي؟**"
          " ترقبوا قريباً إطلاق النسخة المدفوعة (PRO) بميزات الذكاء الاصطناعي"
          " المتقدم!"
)
